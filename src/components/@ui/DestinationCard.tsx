...
export default function DestinationCard({ data , height="h-[260px]"}: DestinationCardProps) {
	return (
		<div className={`relative rounded-xl overflow-hidden group h-[260px] bg-black ${height}`}>
			<Image
				src={img}
				fill
			...
			<div className="absolute inset-0 p-5 text-center space-y-3 flex flex-col items-center justify-center text-white group-hover:translate-y-0 translate-y-[50%] duration-200">
				...
				<p className={`opacity-0 group-hover:opacity-100 font-sans ${height? "max-w-[320px]":""}`}>{data.shortInfo}</p>
